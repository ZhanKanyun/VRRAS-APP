...
namespace HYT.MidiManager
{
    ...
    internal class TrackReader
    {
        ...
        public void Read(Stream strm)
        {
            ...
            //读取每拍毫秒数 FF5103 后面三个字节
            for (int i = 0; i < trackData.Length; i++)
            {
                byte b = trackData[i];
                if (b == 0xFF && i < trackData.Length - 6)
                {
                    if (trackData[i + 1] == 0x51 && trackData[i + 2] == 0x03)
                    {
                        newTrack.Tempo = trackData[i + 3] * 256 * 256 + trackData[i + 4] * 256 + trackData[i + 5];
